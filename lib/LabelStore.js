...
class LabelStore {
	...
	async getLabelsToCreateAndUpdate (source, target) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				if (labelsToCreate.length) {
					...
					if (await prompt.askYesNo('You want to create these labels')) {
						createLabelDefinitions = R.filter(label =>  R.contains(label.name, labelsToCreate), source.labels);
					}
				}
				if (labelsToUpdate.length) {
					console.log(`\nThese labels aready exist but with another color: ${labelsToUpdate.join(', ')}\n`);
					if(await prompt.askYesNo('You want to update the color of these labels')) {
